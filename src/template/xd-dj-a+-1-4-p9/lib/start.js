...
export default class Start extends Common {
  ...
  aniFun(i) {
    this.aniArr[i].visible = true
    this.btnArr.map(v => this._stage.addChild(v))
    this.aniArr[i].state.setAnimation(0, 'in', false).listener = {
      complete: () => {
        this.aniArr[i].visible = false

        this.eqArr[i].visible = true
        this.frameArr[i].visible = true

        this.markArr.map((v, j) => {
          if (i === 0 && j < 4) v.visible = true
          if (i === 1 && j < 8) v.visible = true
          if (i === 2 && j >= 8) v.visible = true
        })
